document.addEventListener("DOMContentLoaded", () => {
  let scorePlayer = 0;
  let scoreEnemy = 0;
  const colors = ["#FD0000", "#0018FD" , "#00FD0B" , "#CF00FD" , "#FD008A", "#000000"]
  ...
  function moveEnemyPaddle() {
    const enemyPaddleCenter = enemyPaddle.y + enemyPaddle.height / 2;

    if (ball.y < enemyPaddleCenter) {
      enemyPaddle.y -= enemyPaddle.dy;
    } else if (ball.y > enemyPaddleCenter) {
      enemyPaddle.y += enemyPaddle.dy;
    }

    if (enemyPaddle.y < 0) {
      enemyPaddle.y = 0;
    } else if (enemyPaddle.y + enemyPaddle.height >= canvas.height) {
      enemyPaddle.y = canvas.height - enemyPaddle.height;
    }
  }

  function createEnemyPaddle() {
    context.beginPath();
    context.rect(enemyPaddle.x, enemyPaddle.y, enemyPaddle.width, enemyPaddle.height);
    context.fillStyle = "#fff";
    context.fill();
    context.closePath();
  }
